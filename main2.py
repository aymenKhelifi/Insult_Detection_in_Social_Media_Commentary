...
CHAR_NGRAMS_RANK_FILE = DIR_PATH + "char_ngrams_rank2.csv"
CHAR_NGRAMS_VALUES_FILE=DIR_PATH + "char_ngrams2.csv"
...
def train(train_data,labels,selected_char_features, selected_features):
    ...
    for comment in train_data:
        print(i)
        occurences[i]=([comment.split(' ').count(x) for x in selected_features])
...
def test(test_data,selected_char_features, selected_features,svc):
    ...
    for comment in test_data:
        print(i)
        occurences_test[i] = ([comment.split(' ').count(x) for x in selected_features])
        i += 1
...
def run():
    ...
    selected_char_features, selected_features=feature_extraction_selecttion(train_data,train_labels,350)
