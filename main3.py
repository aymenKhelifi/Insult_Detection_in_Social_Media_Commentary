...
def countSpecial(sentence):
    a=[]
    a.append(sentence.count("!"))
    a.append(sentence.count("."))
    a.append(sentence.count(">"))
    a.append(sentence.count("<"))
    a.append(sentence.count(":"))
...
def train(train_data,labels,train_data_original,selected_char_features, selected_features):
    ...
    for comment in train_data:
        #print(i)
        temp = [comment.count(x) for x in finalfeatures]
        temp.append(countSpecial(train_data_original[i]))
        occurences[i]=temp
    ...
    #svc=MLPClassifier(solver='lbfgs', alpha=1e-5,activation='tanh',hidden_layer_sizes = (20, 2), random_state = 1)
    svc.fit(occurences,labels)
    return svc

def test(test_data,test_sol_data_original,selected_char_features, selected_features,svc):
    ...
    for comment in test_data:
        #print(i)
        temp=[comment.count(x) for x in finalfeatures]
        temp.append(countSpecial(test_sol_data_original[i]))
        occurences_test[i]=temp
        i += 1
...
def run():
    ...
    test_sol_data_original=test_sol_data
    ...
    selected_char_features, selected_features=feature_extraction_selecttion(train_data,train_labels,750)
    svc=train(train_data,train_labels,train_data_original,selected_char_features, selected_features)
    print("Testing\n")
    predicted=test(test_sol_data,test_sol_data_original,selected_char_features, selected_features,svc)
    print("Evaluating\n")
